...
def show_cam_on_image(img, mask, name,  bj , mark):
	heatmap = cv2.applyColorMap(np.uint8(255 * mask), cv2.COLORMAP_JET)
	heatmap = np.float32(heatmap) / 255
	cam = heatmap + np.float32(img)
	cam = cam / np.max(cam)
	if mark == bj :
		cv2.imwrite("1-1/cam_{}.jpg".format(name), np.uint8(255 * cam))
	else :
		cv2.imwrite("1-2/cam_{}.jpg".format(name), np.uint8(255 * cam))
...
if __name__ == '__main__':
	...
	for img in image:
		...
		if i == 0:
			mark = predicted_idx.item

		mask = grad_cam(input, target_index)
		i = i + 1
		show_cam_on_image(img, mask, i, mark, predicted_idx.item)
